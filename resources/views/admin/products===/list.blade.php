...
<div id="content_list">@include('admin.products.list.content')</div>
...
//       $('#adminForm img#image_'+selc+'_img').attr('src', res); $('#adminForm img#image_'+selc+'_img').show();
//       $('#adminForm input#image_'+selc+'_img').attr('value', res);
...
//   });
    ...
    $(document).ready(function(){ 
        ...
        $('body').on('click','#sell_continue',function(){ 
            ...
            if(sellerId > 0){ 
                $.ajax({
                    type: "GET",
                    url: '{{url("admin/product/0")}}/'+sellerId,
    ...
    $('.stars').stars();
         ...
         $('body').on('click','.specialOfr',function(){
            ...
            $.ajax({
                type: "GET",
                url: '{{url("/admin/products/offer/")}}/'+id,
                success: function (data) {
...
required: "Status field is required."
            ...
            var id      =  this.id.replace('modalBtn-','');
            alert(id);
            $.ajax({
                type: "GET",
                url: '{{url("admin/product/review-content")}}/'+id,
                success: function (data) { 
        ...
        $('body').on('click','.viewBtn',function(){
            ...
            $.ajax({
                type: "GET",
               url: '{{url("admin/product")}}/'+id+"/"+sellerId+"/view",
                success: function (data) {
                    $('#content_detail').html(data); $('#content_detail').fadeIn(700); $('#content_list').hide(); 
        ...
        $('body').on('click','.cus_radio',function(){ 
            if(this.id == 'option2'){ 
                ...
                $('#adminForm #category_id').attr('disabled',false);
                $('#adminForm .comboTreeArrowBtn').attr('disabled',false);
                $('#adminForm .comboTreeWrapper').css({"background-color": "#fff","pointer-events":"auto"}); 
                $('#adminForm #sub-category-drop').css({"background-color": "#fff"}); 
            }
        });
                ...
                success: function (data) { 
                    $.each(data, function(key,value) { // alert(key+' -- '+value);
                        if(key != 'id'){ $('#adminForm #'+key).val(value); }
                           if(key == "category_id") { $('#adminForm #categoryList').val(value); $('#adminForm #category_id').attr('disabled',true);  }
                        if(key == "sub_category_id") { $('#adminForm #sub-category-id').val(value); loadsubcat('1'); $('#adminForm .comboTreeArrowBtn').attr('disabled',true); 
                        
                        setTimeout(function() { 
                        $('#adminForm .comboTreeWrapper').css({"background-color": "#e9e9eb","pointer-events":"none"}); 
                        $('#adminForm #sub-category-drop').css({"background-color": "#e9e9eb"}); 
                ...
                if($('#adminForm #can_submit').val() > 0){ return true; }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            //CHECKBOX VALIDATION
                            if ($("#adminForm input:checkbox:checked").length > 0){ }
                                  else {
                                    $('.error-field').show();
        ...
        $("body").on("change", ".status-btn", function () {
            ...
            var url         =   '{{url("admin/product/updateStatus")}}';
        ...
        $('body').on('click','.delBtn',function(){  // alert('sss');
            ...
            var status      =   1;
            var url         =   '{{url("admin/product/updateStatus")}}';
            var smsg        =   'Seller product deleted successfully!';
            swal({
                title: "Delete Confirmation",
                text: "Are you sure you want to delete this Seller Product?",
                // type: "input",
        ...
        $('body').on('click','#add_more',function(){ 
            ...
            htmlContent                 =   htmlContent.replace('image_disp_name','image_'+row+'_img');
