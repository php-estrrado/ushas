    ...
    $(document).ready(function(){ 
        ...
        $("body").on("change", ".status-btn", function () {
           ...
           $('body').on('click','.delBtn',function(){  // alert('sss');
            ...
            var status      =   1;
            var url         =   '{{url("admin/seller/product/updateStatus")}}';
            var smsg        =   'Seller product request deleted successfully!';
            swal({
                title: "Delete Confirmation",
                text: "Are you sure you want to delete this Seller Product Request?",
                // type: "input",
   ...
   $('#attr_'+row).modal('show');
    ...
    function updateStatus(id,rowId,status,url,row,field,smsg){
        $.ajax({
            ...
            success: function (data) {
                if(field == 'is_deleted'){  setTimeout(function(){ location.reload(); }, 1000); }
