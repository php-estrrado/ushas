						...
						<div class="page-header">
							<div class="page-leftheader">
								...
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item " aria-current="page"><a href="{{url('/admin/occasions')}}">Occasions</a></li>
...
{{ Form::open(array('url' => "admin/occasions/save", 'id' => 'occasionsForm', 'name' => 'occasionsForm', 'class' => '','files'=>'true')) }}

												<input type="hidden" name="id" value="{{ $occasion['id'] }}">
												<input type="hidden" name="occasion_name_cid" value="{{ $occasion['occasion_name_cid'] }}">
...
@php
  $def_lang =DB::table('glo_lang_lk')->where('is_active', 1)->first();
        $content_table=DB::table('cms_content')->where('cnt_id', $occasion['occasion_name_cid'])->where('lang_id', $def_lang->id)->first();
        if($content_table){ 
        $lang_id = $content_table->lang_id;
         ...
         @endphp
																...
																<div class="row">
																	<div class="col">
																		<div class="form-group">
																			...
																			<input type="text"  class="form-control" name="occasion" value="{{ $occasion['occasion'] }}">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="d-flex">
                                                                <img src="{{ url('uploads/storage/app/public/occasions/'.$occasion['image']) }}" alt="{{ $occasion['image'] }}"  style="height: 150px; max-height:150px; width:auto;">
                                                                <input type="hidden" value="{{ $occasion['image'] }}" name="image_file">
                                                            </div>
                                                        </div>
                                                        </div>
                                                        @endif
														<div class="row">
                                                           ...
                                                           <label class="form-label">Choose another occasion image <span class="text-red">*</span></label>
                                                           <p>(Image type .png,.jpeg)</p>
                                                           <input type="file" class="dropify @error('occasion_image') is-invalid @enderror" data-height="180"  accept="image/*" name="occasion_image" data-allowed-file-extensions='["png", "jpg", "jpeg"]' />
                                                          
                                                        </div>
                                                      
															   @error('occasion_image')
																	<p style="color: red">{{ $message }}</p>
																	@enderror
																</div>
																<div class="row">
																<div class="col">
																		<div class="form-group">
																			...
																			{!! Form::select('is_active', array('1' => 'Active', '0' => 'Inactive'), $occasion['is_active'],['class' => 'form-control','required','id'=>'module_status']); !!}
																		</div>
																	</div>
																</div>
															</div>
														...
														<div class="row" style="margin-top: 30px;">
															<div class="col d-flex justify-content-end">
															     <a href="{{url('/admin/occasions')}}"  class="mr-2 btn btn-secondary" >Cancel</a>  
