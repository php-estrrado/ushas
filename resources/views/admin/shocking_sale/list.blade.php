						...
						<div class="page-header">
							<div class="page-leftheader">
								...
								<div class="btn btn-list">
									...
									<a href="{{ url('admin/shocking-sales/create/') }}"  class="btn btn-primary addmodule"><i class="fe fe-plus mr-1"></i> Add New</a>
								</div>
							</div>
																	...
																	<span class="switch-inactive" aria-hidden="true">Inactive</span>
																	 ...
																	 <td class="align-middle">
																		<div class="btn-group align-top">
																			@if(checkPermission('/admin/shocking-sales','edit') == true)
																			<a href="{{ url('admin/shocking-sales/edit/') }}/{{ $row['id'] }}"   class="mr-2 btn btn-info btn-sm editmodule"><i class="fe fe-edit mr-1"></i> Edit</a>
																			@endif
																			@if(checkPermission('/admin/shocking-sales','delete') == true)
																			<button  class="btn btn-secondary btn-sm deletemodule" onclick="deletecpn({{$row['id']}},{{$row['deletable']}});" type="button"><i class="fe fe-trash-2  mr-1"></i>Delete</button>
																			@endif
																		</div>
																	</td> 
																</tr>
...
@section('js')
		...
		<script src="{{URL::asset('admin/assets/js/datatable/tables/shockingsales-datatable.js')}}"></script>
...
if($deletable == 1) {
		...
		$.ajax({
		type: "POST",
		url: '{{url("/admin/shocking-sales/delete")}}',
...
$('body').on('change','.ser_status',function(){  
        ...
        $.ajax({
        type: "POST",
        url: '{{url("/admin/shocking-sales/status")}}',
