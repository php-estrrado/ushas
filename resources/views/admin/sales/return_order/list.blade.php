<style> .no-border{ border: none } .invoice i.fa{ color: #ff0000; font-sze: 10px; } </style>
...
<div class="page-header">
    <div class="page-leftheader">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item view-li no-disp"><a id="bc_list" href="">Return Orders</a></li>
...
<div id="content_list">@include('admin.sales.return_order.list.content')</div>
...
<script>
    ...
    $(document).ready(function(){ 
        ...
        $('body').on('click','.viewDtl',function(){ 
            ...
            $.ajax({
                type: "GET",
               url: '{{url("admin/sales/return/order")}}/'+id,
...
alert(id);			
        ...
        $('body').on('click','#accept_btn',function(){ 
            var id          =   $(this).data('val');  
            var status      =   'accepted';
            var url         =   '{{url("admin/sales/order/return/updateStatus")}}';
            var smsg        =   'Request '+status+' successfully!';
            var desc        =   'Return order accepted by Admin';
            swal({
                title: "Order return Confirmation",
                text: "Are you sure want to return this order?",
                // type: "input",
                ...
                if (inputValue == true) { 
                    updateStatus(id,'order_return',status,url,'sales.return_order','status',smsg,desc);
                }
            });
        });
        ...
        $('body').on('click','#reject_btn',function(){ 
            ...
            var url         =   '{{url("admin/sales/order/return/updateStatus")}}';
            var smsg        =   'Request '+status+' successfully!';
            var desc        =   'Return order rejected by Admin';
            swal({
                title: "Order return reject confirmation",
                text: "Are you sure want to reject this return order?",
                // type: "input",
                showCancelButton: true,
                closeOnConfirm: true,
                confirmButtonText: 'Yes'
        ...
        $('body').on('click','#ship_reject_btn',function(){ 
            var id          =   $(this).data('val');  
            var status      =   'rejected';
            var url         =   '{{url("admin/sales/order/return/updateStatus")}}';
            var smsg        =   'Request '+status+' successfully!';
            var desc        =   'Return order rejected';
            swal({
                title: "Order shipment reject confirmation",
                text: "Are you sure want to reject this shipment?",
                // type: "input",
                showCancelButton: true,
                closeOnConfirm: true,
                confirmButtonText: 'Yes'
                ...
                if (inputValue == true) { 
                    updateStatus(id,'order_shipment',status,url,'sales.return_order','status',smsg,desc);
                }
            });
        });
        
        $('body').on('click','#ship_accept_btn',function(){ 
            var id          =   $(this).data('val');  
            var status      =   'accepted';
            var url         =   '{{url("admin/sales/order/return/updateStatus")}}';
            var smsg        =   'Request '+status+' successfully!';
            var desc        =   'Return order accepted';
            swal({
                title: "Order return received confirmation",
                text: "Are you sure want to accept this shipment?",
                // type: "input",
                showCancelButton: true,
                closeOnConfirm: true,
                confirmButtonText: 'Yes'
        ...
        $('body').on('click', '#reply_submit', function(){
           if($('.modal-content #reply').val() == ''){ $('.modal-content #reply_error').text('Comment field is required'); return false; }else{ $('.modal-content #reply_error').text(''); }
            ...
            var smsg        =   'Request '+status+' successfully!';
            var desc        =   'Cancellation rejected by Admin';
            updateStatus(id,'order_cancels',status,url,'sales.return_order','status',smsg,desc); return false;
        });
        ...
        $('body').on('change','#start_date',function(){  
            ...
            $.ajax({
                type: "GET",
                url: '{{url("admin/sales/return/orders")}}',
        ...
        $('body').on('change','#end_date',function(){  
            ...
            $.ajax({
                type: "GET",
                url: '{{url("admin/sales/return/orders/$type")}}',
