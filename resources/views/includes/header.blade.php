...
<!--									<div class="mt-1">
									...
									<div class="d-flex order-lg-2 ml-auto">
										...
										$cmsg = ChatMessage::getChatMsg();@endphp
										<div class="dropdown header-message">
											<a class="nav-link icon" data-toggle="dropdown">
												...
												<span class="badge badge-success side-badge">{{count($cmsg)}}</span>
											</a>
											<div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow  animated">
												<div class="dropdown-header">
													<h6 class="mb-0">Messages</h6>
													<span class="badge badge-pill badge-primary ml-auto"><a href="{{url('admin/chat/list')}}" class="text-white">View all</a></span>
												</div>
												@if(count($cmsg)>0)
												@php $i=1;@endphp
												<div class="header-dropdown-list message-menu" id="message-menu">
												@foreach($cmsg as $rowmsg)
												
													<a class="dropdown-item border-bottom" href="{{url('admin/chat/list')}}">
														<div class="d-flex align-items-center">
															<div class="">
																<span class="avatar avatar-md brround align-self-center cover-image" data-image-src="{{$rowmsg['img']}}"></span>
															</div>
															<div class="d-flex">
																<div class="pl-3">
																	<h6 class="mb-1">{{$rowmsg['name']}}</h6>
																	@if($rowmsg['msg_type']=='text')
																	<p class="fs-13 mb-1">{{$rowmsg['msg']}}</p>
																	@else
												...
												<div class=" text-center p-2 border-top">
													<a href="{{url('admin/chat/list')}}" class="">See All Messages</a>
												</div>
											</div>
										</div>
										@php $notifications=AdminNotification(5); $i=0; $newnotification=0; @endphp
										@if($notifications) 
											@foreach($notifications as $notify)
											@if($notify->viewed==0)
										    @php $newnotification=1 @endphp
											@endif
											@endforeach
