...
class OdooProductController extends Controller
{
        ...
        if ($validator->passes()){
          ...
          //  $videos = $request->file('video');
            ...
            if($request->specification)
            {
                ...
                $spec_cont = array('ops'=>[array("insert"=>$request->specification)]);
                $spec_cont = json_encode($spec_cont);
            $prd['spec_cnt_id']       = $this->addCmsContent($product->spec_cnt_id,1,$spec_cont);    
            }
     ...
     foreach($request->attrs as $row)
    {
      ...
      //Combination array
      if(isset($request->combination)){ //dd($request->combination); 
