...
class CustomerWallet extends Controller
{
   ...
   public function index()
   {
    ...
    $data['wallet']             =   CustomerWallet_Model::select(DB::raw("user_id,SUM(credit)-SUM(debit) as wallet"))->orderBy("id","DESC")->groupBy(DB::raw("user_id"))->get();
   return view('customer.wallet.list', $data);
   }
   
   public function wallet_log($cus_id)
   {
        $data['wallet']           = CustomerWallet_Model::select(DB::raw("user_id,SUM(credit)-SUM(debit) as wallet"))->where('user_id',$cus_id)->where('is_deleted',0)->orderBy("id")->groupBy(DB::raw("user_id"))->first();
