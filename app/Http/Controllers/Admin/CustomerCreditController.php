...
class CustomerCreditController extends Controller
{
   ...
   {
    $data['title']              =   'Customer Credits';
    $data['menu']               =   'Customer Credits List';
   ...
   ->join("$credit_log_table as log",'log.id','=',"$credit_table.log_id")->where("log.is_deleted",0)->orderBy("$credit_table.id","DESC")->groupBy("$credit_table.user_id")
   ->get();
    // dd($data);
   return view('customer.credits.list', $data);
   }
   
   public function credits_log($cus_id)
   {
        ...
        ->first();
        $data['transaction']      = CustomerCredits::where('user_id',$cus_id)->orderBy('id','DESC')->get();
        $data['customer']         = CustomerInfo::where('user_id',$cus_id)->first();  
        // dd($data);
        return view('customer.credits.log_view', $data);
 ...
 public function manage_credits($cus_id)
   {
        ...
        $data['customer']         = CustomerInfo::where('user_id',$cus_id)->first();  
        // dd($data);
        return view('customer.credits.manage_credits', $data);
   ...
   $query->select('user_id')->from("$credit_log_table");})->get();
        ...
        return view('customer.credits.add_credits', $data);
    ...
    public function payment_form(Request $request)
    {
        ...
        return view('customer.credits.payment_form', $data);
    }
