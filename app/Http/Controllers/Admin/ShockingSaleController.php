...
class ShockingSaleController extends Controller{
    ...
    { 
     ...
     function createShockingSale(){
        ...
        $data['shockingproducts']      = PrdShockingSaleProduct::where('is_active', 1)->where('is_deleted', 0)->pluck('prd_id');
       	$data['products']				= 	 Product::whereNotIn('id',$data['shockingproducts'])->where('is_deleted','0')->where('is_active','1')->where('visible','1')->get();
		//$data['products']					= 	 Product::whereNotIn('id',$config_visible)->where('is_deleted','0')->where('is_active','1')->get();
        ...
        $data['shockingproducts']      = PrdShockingSaleProduct::where('shock_sale_id',$sale_id)->where('is_active', 1)->where('is_deleted', 0)->pluck('prd_id');
        $shockingproducts      = PrdShockingSaleProduct::where('shock_sale_id','!=',$sale_id)->where('is_active', 1)->where('is_deleted', 0)->pluck('prd_id');
		//$config_visible				= 	 Product::where('is_deleted','0')->where('is_active','1')->where('visible','1')->pluck('id');

		$data['products']					= getDropdownData(Product::whereNotIn('id',$shockingproducts)->where('visible','1')->where('is_deleted','0')->where('is_active',1)->get(),'id','name'); 
		//Product::whereNotIn('id',$config_visible)->where('is_deleted','0')->where('is_active','1')->get();
...
$sale_arr['updated_at'] = date("Y-m-d H:i:s");
		...
		PrdShockingSaleProduct::where('shock_sale_id',$input['id'])->update(['is_deleted' => '1','is_active' => '0']);
		if($input['prd_id']){
			
			   foreach($input['prd_id'] as $row){	
				$product_detals['prd_id']=$row;
				$product_detals['shock_sale_id']= $input['id'];
				$product_detals['is_active']=1;
				PrdShockingSaleProduct::create($product_detals)->id;
			   }
		}
        }else {
        ...
        $saleId                  =   PrdShockingSale::create($sale_arr)->id;
         $msg    =   'Shocking Sale added successfully!';
		  if($input['prd_id']){
			   foreach($input['prd_id'] as $row){	
				$product_detals['prd_id']=$row;
				$product_detals['shock_sale_id']= $saleId;
				$product_detals['is_active']=1;
				PrdShockingSaleProduct::create($product_detals)->id;
			   }
			}
