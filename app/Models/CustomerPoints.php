...
class CustomerPoints extends Model
{
    ...
    {
        $bal =0;
        $balance = CustomerPoints::where('user_id',$user_id)->selectRaw('SUM(credit) - SUM(debit) as bal')->first()->bal;
        if($balance>0)
        {
            return $balance;
        }
    ...
    {
        $data = CustomerPoints::where('user_id',$user_id)->get();
        $pt = [];
        foreach($data as $row)
        {
	...
	static function getListData($post) {     $res = []; 
        $CustomerPointsModel = new CustomerPoints; 
		$userModel = new customer\CustomerInfo;
		//$masterModel = new customer\CustomerMaster;
        $LoyaltyLog  =   $CustomerPointsModel->from($CustomerPointsModel->getTable().' as P')->join($userModel->getTable().' as T','P.user_id','=','T.id')->where('P.is_deleted',0)->
		...
		//CustomerPoints::getCredits($row->usr_addr_typ_id);
