...
'valid_from','valid_to','valid_days','is_active','is_deleted','created_by','user_type','updated_by','created_at','updated_at','image','unique_id'];
        ...
        //Active coupons
        static function getActiveCoupons($cpn='',$search=''){ 
            if($cpn) {
                $cpn_list = Coupon::where(function ($query) { $query->where('is_deleted', '=', NULL)->orWhere('is_deleted', '=', 0);})->where('is_active',1)->where('id',$cpn)->orderBy('id', 'DESC')->get();
            }else {
           $cpn_list = Coupon::where(function ($query) { $query->where('is_deleted', '=', NULL)->orWhere('is_deleted', '=', 0);})->where('is_active',1);
           if($search)
           {
            $cpn_list=$cpn_list->whereIn('cpn_title_cid',function($query) use ($search){$query->select('cnt_id')->from('cms_content')->where('content', 'like', "%{$search}%");});
           }
           $cpn_list=$cpn_list->orderBy('id', 'DESC')->get();     
            }
        ...
        // all Active-inactive/deleted coupons
        static function getAllCoupons($cpn=''){ 
            if($cpn) {
                $cpn_list = Coupon::where('id',$cpn)->orderBy('id', 'DESC')->get();
            }else {
           $cpn_list = Coupon::orderBy('id', 'DESC')->get();     
            }
        ...
        static function getCouponData($cpn_id){ 
                
           $cpn_list = Coupon::where("id",$cpn_id)->orderBy('id', 'DESC')->get();     
            ...
            foreach($cpn_list    as  $row){
            $data['id']        =   $row->id;
            $data['cpn_title']         =   Coupon::getCpnContent($row->cpn_title_cid);
            $data['cpn_title_cid']         =   $row->cpn_title_cid;
            $data['cpn_desc']       =    Coupon::getCpnContent($row->cpn_desc_cid);
            $data['cpn_desc_cid']         =   $row->cpn_desc_cid;
            $data['cat_name']       =    Coupon::getCpnCat($row->category_id);
            $data['category_id']         =   $row->category_id;
             $data['subcategory_id']         =   $row->subcategory_id;
            if($row->subcategory_id) {
            $data['subcat_name']       =    Coupon::getCpnSubCat($row->subcategory_id);
            }else {
            $data['subcat_name']       =    "-";
            } 
            $data['image']           =   $row->image;
            $data['seller_id']       =   $row->seller_id;
            $data['purchase_type']       =   $row->purchase_type;
            $data['purchase_number']       =   $row->purchase_number;
            $data['purchase_amount']       =   $row->purchase_amount;
            $data['ofr_value_type']       =   $row->ofr_value_type;
            $data['ofr_value']       =   $row->ofr_value; 
            $data['ofr_type']       =   $row->ofr_type; 
            $data['ofr_code']       =   $row->ofr_code; 
            $data['ofr_min_amount']       =   $row->ofr_min_amount; 
            $data['validity_type']       =   $row->validity_type;
            $data['valid_from']       =   $row->valid_from;
            $data['valid_to']       =   $row->valid_to;
