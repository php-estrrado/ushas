...
<div class="page-header">
    ...
    <div class="page-rightheader d-none" style="display:flex; flex-direction: row; justify-content: center; align-items: center">
        ...
        <?php echo e(Form::select('active',['Electronics'=>'Electronics','Grocery'=>'Grocery'],'',['id'=>'status_filter','class'=>'form-control mr-4','placeholder'=>'All Status'])); ?>
...
<div id="filters">
    ...
    <div class="row no-disp mb-4 " id="filtersec">
        ...
        <div class="col-2 fl">
            <div class="page-filters">
                <?php echo e(Form::label('p_status','Payment Status',['class'=>'text-white'])); ?>

                <?php echo e(Form::select('p_status',$pStatusList,$p_status,['id'=>'p_status','class'=>'form-control','placeholder'=>'All Status'])); ?>

            </div>
        </div>
        <div class="col-2 fl">
            <div class="page-filters">
                <?php echo e(Form::label('o_status','Order Status',['class'=>'text-white'])); ?>

                <?php echo e(Form::select('o_status',$orderStatusList,$o_status,['id'=>'o_status','class'=>'form-control','placeholder'=>'All Status'])); ?>
...
<div id="content_list"><?php echo $__env->make('admin.sales.order.list.content', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?></div>
...
<script>
    ...
    $(document).ready(function(){ 
        ...
        $('body').on('click','.viewDtl',function(){ 
            ...
            $.ajax({
                type: "GET",
               url: '<?php echo e(url("admin/sales/order")); ?>/'+id+"/request",
        ...
        $('body').on('click','.editBtn',function(){
            ...
            $.ajax({
                type: "GET",
               url: '<?php echo e(url("admin/sales/invoice")); ?>/'+id,
                success: function (data) {
                    $('.list-li').hide(); $('.view-li').fadeIn(700); $('#filters').hide();
                    $('#content_detail').html(data); $('#content_detail').fadeIn(700); $('#content_list').hide(); 
                } 
        ...
        $('body').on('click','#content_detail #add_btn',function(){
            ...
            $.ajax({
                type: "GET",
                url: '<?php echo e(url("/admin/product")); ?>/'+id+'/'+sId+'/add',
        ...
        $('body').on('change','#start_date',function(){  
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo e(url("admin/sales/orders")); ?>',
        ...
        $('body').on('click','.actBtn',function(){
            ...
            var url         =   '<?php echo e(url("admin/sales/order/updateStatus")); ?>';
		 ...
		 $('body').on('click','.OrderactBtn',function(){
            var id          =   $('#orderId').val();
            var status      =   $("#order_status").val();
            var url         =   '<?php echo e(url("admin/sales/order/update-status")); ?>';
            var smsg        =   'Order Status Changed successfully!';
            swal({
                title: "Order status change Confirmation",
                text: "Are you sure want to update order status?",
                // type: "input",
                ...
                if (inputValue == true) { 
                    updateOrderStatus(id,'',status,url,'sales.order_request','order_status',smsg);
                }
            });
        });
    ...
    function updateOrderStatus(id,rowId,status,url,row,field,smsg){
        $.ajax({
            type: "POST",
            url: url,
            data: { "_token": "<?php echo e(csrf_token()); ?>", id: id, value: status,field: field },
            success: function (data) {
				if(data==1){
					
				$("#orderStatustxt").html(status).css('textTransform', 'uppercase');

              toastr.success(smsg);  return false; 
				}else{
...
<?php /**PATH /home/qaushas/public_html/resources/views/admin/sales/order/list.blade.php ENDPATH**/ ?>